...
class AgentStateLayer(nn.Module):
    def __init__(self) -> None:
        ...
        self.out_dim = 128
        self.linear_layer, self.linear_layer_out_dim = linear_layer([None, self.agent_state_dim], self.out_dim) # 输出一个长为128的dim
...
class Model(nn.Module):
    ...
    def forward(self, states): 
        ...
        if len(states) > 7: # action mask for rl training
            action_mask = states[7].float() # 即legal actions可执行的操作
        global_feature = self.global_state_layer(global_feature) # 先对每个拉长拼成的特征进一个全连接
