...
class Policy(nn.Module):
    ...
    def update(self, states, actions):
        states = [torch.tensor(np.array(state),dtype=torch.float32).to('cuda') for state in states]
...
if __name__ == '__main__':
    ...
    print(file_pointers[:2])
    for epoch in range(num_epochs):
        # try: # avoid file read error
        states_batch, action_batch = sample_batch(file_pointers,wrapper)
        print(f"action_batch: ", len(action_batch))
        print(f"states_batch: ", len(states_batch))
        for i, states in enumerate(states_batch):
            print(i, "states: ", states.shape)
            
        # for i, action in enumerate(action_batch):
            # print(i, "action: ", action.shape)
        break
