...
class Policy:
    ...
    def sgd_iter(self, states, actions, returns, old_log_probs):
        ...
        for _ in range(batch_size//mini_batch_size):
            ...
            # random 32 samples  between 0 and batch_size.
    ...
    def update(self, stats_recorder=None):
        ...
        for _ in range(2):
            # iter-num :batch_size//mini_batch_size
            for states, actions, returns, old_log_probs in self.sgd_iter(states, actions, returns, old_log_probs):
                # states is a list which include size 30, 73, 73,73,73,73, 73, 52
...
class Memory():
    ...
    def handle(self, exps):
        # exp.state is a list including: global-states, self-states, ally0-states, ally1-states,
        # enemy0-states, enemy1-states, enemy2-states,action_mask
        states = [[] for _ in range(8)]
        #8*512*states_size(30,73...)
        for exp in exps:
            for i in range(8):
...
def train(env, policy,stats_recorder=None):
    ...
    for _ in range(1000000):
        ...
        # states: {0: [global, self,....],1:[global, self,....],...,5:[global, self,....]}
        ep_rewards = defaultdict(int)
        ep_step = 0
        while True:
            # actions: [0:int(0-51),1: int(0-51), ...,5:int(0-51)]
            ...
            for key in share_keys:
                ...
                # make a exps includ six agent's exp
                exps.append(exp)
            # for 86x6 = 516，if can be execute
            if len(exps) >= 512:
...
if __name__ == '__main__':
    ...
    # test(env, policy)
    # six agent share the same actor-critic net,in addition actor and critic share the same net.  
    # upgrad:
        
        # 1 seperate actor net , critic net
        # 2 make two, self model, enemy model, do not need to change input,just 
        #   frozen the weight of enemy,update the weight of self ally
