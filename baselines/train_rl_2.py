    ...
    while True:
        ...
        while not exps: # exps is empty
            for collector_id, data_queue in enumerate(data_queues):
                ...
                try:
                    while True:
                        # donot block throw exception instaniously
        ...
        # model weights updates meanwhile collect data
        for exp in exps:
            ep_len = exp.obs.shape[0]
            ...
            _, probss = model(state_input) # new_values shape: [9, 1] probs shape: [9, 52]
        ...
        if global_step - last_save_step > 100000:
            ...
            last_save_step = global_step
        # exps finished,model weights will put to model_queue.
        model_cpu = {k: v.cpu() for k, v in model.state_dict().items()}
        value_model_cpu = {k: v.cpu() for k, v in value_model.state_dict().items()}
        # 每一个进程里面都要用到model_cpu
