...
class BaseEnv():
    ...
    def reset(self, user_name = None, render = True):
        ...
        while True:
            # step1 start tcp server 
            # step2 start game client server and it will build tcp connection with tcp server
            # step3 wait for state
            self._start_all()
    ...
    def _start_client(self):
        ...
        if self.env_setting == 'win':
            # run game client
            # 训练服务器会启动一个server，并暴漏端口。游戏客户端作为tcp协议下的client,发送数据给server
